...
//   // ...
//
//   // Get an API handle to reddit for a logged out (script) program, which
//   // forwards this user agent on all requests and issues a request at most
//   // every 5 seconds.
//   apiHandle := reddit.NewScript("your user agent", 5 * time.Second)
//
//   // Create a configuration specifying the event sources on Reddit that graw
...
//   // at any time, and 'wait' will block until the scan finishes.
